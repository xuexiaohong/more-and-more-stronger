...
```
网格布局:很多网页是基于网格设计的，这说明网页时按列来布局的。使用网格有助于设计网页，更简单的添加网页元素。

响应式网格视图通常时12列，宽度100%，在浏览器窗口大小调整时会自动伸缩。
12 列的网格系统可以更好的控制响应式网页。
使用：确保所有的html元素都有box-sizing属性且设置值为border-box，确保边距和边框都包含在元素的宽度和高度间。
* {
    box-sizing: border-box;
}

媒体设计：使用行和列来制作网页，它是响应式的，但在小屏幕上并不能友好的展示。
